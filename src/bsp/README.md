...
The board support packages provides:
...
This folder contains the subset of board support packages for all of the supported MCU platforms. All code generated in each subsequent directory is source controlled as the project BSP which is derived from the CubeMX `.ioc` file which describes the pinouts for the MCU. This ensures that the BSP and hardware configuration is in lockstep for each release.
